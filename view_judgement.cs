...
{
    public partial class view_judgement : Form
    {
        ...
        private void dytm()
        {
            ...
            string Sql = "SELECT slno,jdid as 'Judgement ID',crid as 'Charge Sheet ID',firid as 'FIR No.',comid as 'Complaint ID',sname as 'Suspect Name',jdate as 'Date',courtname as 'Court Name',jdstatus as 'Status',pgiven as 'Punishment Given' from tbl_judgmentdetails";
        ...
        private void searc()
        {
            //On Error Resume Next
            conn = new SqlConnection(ConnString);
            string Sql = "SELECT slno,jdid as 'Judgement ID',crid as 'Charge Sheet ID',firid as 'FIR No.',comid as 'Complaint ID',sname as 'Suspect Name',jdate as 'Date',courtname as 'Court Name',jdstatus as 'Status',pgiven as 'Punishment Given' from tbl_judgmentdetails where jdid like '%" + TextBox1.Text + "%' order by jdid";
            da2 = new SqlDataAdapter(Sql, conn);
            conn.Open();
            ds2 = new DataSet();
            SqlCommandBuilder commandBuilder1 = new SqlCommandBuilder(da2);
        ...
        private void DataGridView3_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (p1 == 0)
            {
                judgementdetails r = null;

                foreach (Form f in Application.OpenForms)
                {
                    if (f is judgementdetails)
                    {
                        r = (judgementdetails)f;
                        break;
                    }
                }
                ...
                r.dpdob.Text = row.Cells[6].Value.ToString();
                r.textBox5.Text = row.Cells[7].Value.ToString();
                r.comboBox2.Text = row.Cells[8].Value.ToString();
        ...
        private void view_judgement_Load(object sender, EventArgs e)
        {
            dytm();
        }
